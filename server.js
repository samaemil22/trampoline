...
// Export module for Vercel Serverless Function
module.exports = app;
*/
/*
const express = require('express');
...
// Configure CORS for GitHub Pages and Localhost
app.use(cors({
    origin: [
        'https://samaemil22.github.io',
        'http://localhost:5000',
        'http://127.0.0.1:5500',
        'http://localhost:3000'
    ],
...
app.get('/', (req, res) => res.send('Backend is running successfully on Vercel!'));
app.get('/api', (req, res) => res.json({ message: 'API is operational' }));
...
app.post('/api/bookings', async (req, res) => {
    ...
    try {
        const values = [
            shift_id || 'SHIFT-1', child_name, father_name, phone || 'لا يوجد',
            game_name, parseInt(duration) || 30, parseInt(player_count) || 1,
            parseFloat(unit_price) || 0, parseFloat(subtotal) || 0,
            parseFloat(discount) || 0, parseFloat(total_price) || 0,
            start_time, end_time, created_by || 'admin'
        ];
        const { rows } = await pool.query(query, values);
        res.status(201).json(rows[0]);
    } catch (err) {
